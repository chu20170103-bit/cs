...
function formatScheduleText(text) {
    ...
    formatted = formatted
        ...
        // 移除所有包含 padolady.pages.dev 的整行
        .replace(/.*?https?:\/\/padolady\.pages\.dev[^\n]*/gi, '')
        // 移除「桃園中壢妹照影」相關文字
        .replace(/.*?桃園中壢妹照影.*?\n?/gi, '')
        // 移除「潘朵拉一線」相關文字
        .replace(/.*?潘朵拉一線.*?\n?/gi, '')
        // 移除「派工請主動告知」相關內容（整個段落）
        .replace(/派工請主動告知[^\n]*生客或熟客[^\n]*謝謝配合[^\n]*/gi, '')
        // 移除時間後可接生客的資訊
        .replace(/\d{2}:\d{2}後可接生客/gi, '')
        // 移除生客驗證相關資訊
        .replace(/可接生客[^\n]*但請提供驗證過紀錄[^\n]*/gi, '')
        .replace(/生客請提供約過別家紀錄[^\n]*/gi, '')
        // 移除網站照片可下載的提示
...
function formatActivityText(text) {
    ...
    lines = lines.filter(line => {
        // 移除包含以下關鍵字的整行
        const keywords = [
            '退水方案', '短工', '長工', '分鐘方案', '退水', '派工數',
            '條件限制', '高退水', '其他妹妹', '活動獎勵', '派工越多',
            '潘朵拉驗證公告', '阿姨或店家', '驗證部份', '驗證方式', 
            '爆工未按', '電話未丟', '生爆熟', '查系統',
            '生客必查號', '點此加入官方', '@113kvvaj',
            'lin.ee/', 'line.me/ti/p/'  // 添加 LINE 連結過濾
        ];
    ...
    formatted = lines.join('\n')
        // 再做一次整段移除（防止跨行內容）
        .replace(/💰[\s\S]*?退水方案[\s\S]*?(?=日期|活動妹妹|$)/gi, '')
        .replace(/📋[\s\S]*?條件限制[\s\S]*?(?=日期|活動妹妹|🔔|$)/gi, '')
        .replace(/🔔[\s\S]*?驗證公告[\s\S]*?(?=日期|活動妹妹|$)/gi, '')
        .replace(/✅[\s\S]*?生客必查[\s\S]*?(?=日期|活動妹妹|$)/gi, '')
        // 清理多餘的空行
        .replace(/\n{3,}/g, '\n\n')
        .replace(/^\s*\n/gm, '')
        .trim();
    ...
    formatted = formatted
        ...
        // 注意：已移除退水方案、條件限制、驗證公告等格式化（客戶版不顯示這些內容）
        // 注意：已移除 LINE 官方連結格式化（客戶版不顯示這些內容）
        // 換行處理
        .replace(/\n/g, '<br>');
...
function formatRulesText(text) {
    ...
    formatted = formatted
        // 移除帳務對帳規範整個段落（包含多行內容）
        .replace(/📋[\s\S]*?帳務對帳規範[\s\S]*?(?=📋|🔔|$)/gi, '')
        // 再次確保移除"帳務對帳規範"相關內容
        .replace(/帳務對帳規範[\s\S]*?(?=📋|🔔|$)/gi, '')
        // 移除帳務誠信處理
        .replace(/📋[\s\S]*?帳務誠信處理[\s\S]*?(?=📋|🔔|$)/gi, '')
        .replace(/帳務誠信處理[^\n]*\n?/gi, '')
        // 移除場內違規處理段落（包含爆工、強劫等）
        .replace(/📋[\s\S]*?場內違規處理[\s\S]*?(?=📋|🔔|$)/gi, '')
        .replace(/場內違規處理[\s\S]*?(?=📋|🔔|$)/gi, '')
        // 移除爆工相關內容
        .replace(/爆工[：:][^\n]*\n?/gi, '')
        .replace(/生客爆工[^\n]*\n?/gi, '')
        .replace(/熟客冒充[^\n]*\n?/gi, '')
        ...
        .replace(/強劫[\/\\]偷竊[^\n]*\n?/gi, '')
        // 移除"若帶同點其他妹爆工"
        .replace(/📋[\s\S]*?若帶同點其他妹爆工[\s\S]*?(?=📋|🔔|$)/gi, '')
        .replace(/若帶同點其他妹[^\n]*\n?/gi, '')
        // 移除其它提醒段落
        .replace(/📋[\s\S]*?其它提醒[\s\S]*?(?=📋|🔔|$)/gi, '')
        .replace(/其它提醒[^\n]*\n?/gi, '')
        // 移除"請多多推薦"
        .replace(/📋[\s\S]*?請多多推薦[\s\S]*?(?=📋|🔔|$)/gi, '')
        .replace(/請多多推薦[^\n]*\n?/gi, '')
        // 移除潘朵拉驗證公告整個段落
        .replace(/🔔[\s\S]*?潘朵拉驗證公告[\s\S]*?(?=📋|🔔|✅|👉|$)/gi, '')
        .replace(/潘朵拉驗證公告[\s\S]*?(?=📋|🔔|✅|👉|$)/gi, '')
        // 移除阿姨或店家派工驗證相關內容
        .replace(/阿姨或店家派工驗證[\s\S]*?(?=📋|🔔|✅|👉|$)/gi, '')
        // 移除生客必查號系統官方
        .replace(/✅[\s\S]*?生客必查號系統官方[\s\S]*?(?=📋|🔔|👉|$)/gi, '')
        .replace(/生客必查號系統[^\n]*\n?/gi, '')
        // 移除"點此加入官方"
        .replace(/👉[\s\S]*?點此加入官方[\s\S]*?(?=📋|🔔|$)/gi, '')
        .replace(/👉[\s\S]*?📱[\s\S]*?點此加入官方[^\n]*\n?/gi, '')
        // 移除單獨的 LINE ID 行（例如：@113kvvaj）
        .replace(/[:：]\s*@[\w]+\s*\n?/gi, '')
        .replace(/@113kvvaj/gi, '')
        // 移除罰款金額相關內容
        .replace(/罰\s*\d+萬元/gi, '')
        .replace(/賠償\s*\d+萬元/gi, '')
        // 清理多餘的空行
        .replace(/\n{3,}/g, '\n\n')
        .replace(/^\s*\n/gm, '\n')
        .trim();
...
function renderGallery() {
    ...
    girlsData.forEach((girl, index) => {
        ...
        // 處理妹妹資訊文字（客戶版：直接顯示 +4 價格）
        const infoTextPlus4 = girl.info.replace(/💲(\d+)🕸(\d+)S回\s*([\d.]+)/g, (match, minutes, shots, basePrice) => {
            const base = parseFloat(basePrice);
            const finalPrice = Math.round((base + 0.4) * 1000);
        ...
        galleryItem.innerHTML = `
            ...
            <div class="girl-content">
                <div class="girl-header">
                    ...
                    <button class="copy-plus4-btn" data-info="${escapeHtml(girl.info)}" title="複製文案">
        ...
        // 添加+4方案複製按鈕事件
        const copyPlus4Btn = galleryItem.querySelector('.copy-plus4-btn');
        if (copyPlus4Btn) {
            copyPlus4Btn.addEventListener('click', (e) => {
                e.stopPropagation();
                copyToClipboardPlus4(girl.info, copyPlus4Btn);
            });
        }
...
// 複製文案到剪貼簿（+4方案版本）
function copyToClipboardPlus4(text, button) {
    // 將 "回 X.X" 格式轉換為 "$XXXX" 格式
    // 公式：(回本 + 0.4) × 1000 = 最終價格
    const plus4Text = text.replace(/💲(\d+)🕸(\d+)S回\s*([\d.]+)/g, (match, minutes, shots, basePrice) => {
        const base = parseFloat(basePrice);
        const finalPrice = Math.round((base + 0.4) * 1000);
        return `💲${minutes}🕸${shots}S $${finalPrice}`;
    });
    
    copyTextToClipboard(plus4Text, button, '✅ 文案已複製！');
}
...
function registerServiceWorker() {
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            ...
            const swPath = './service-worker.js?v=4';
